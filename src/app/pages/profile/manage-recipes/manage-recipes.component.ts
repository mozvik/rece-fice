...
export class ManageRecipesComponent implements OnInit {
  ...
  ngOnInit(): void {
    let target: any[] = [
      {
        recipeName: 'Válassz egy felhasználót a fenti legördülő listából',
        ingredients: '',
        directions: '',
        moderated: '',
        created: '',
    ...
    this.filterGroup.controls['user'].valueChanges.subscribe((value) =>
      this.selectedUserChanged(value)
    );
  }
  ...
  selectedUserChanged(value: any) {
    this.isLoading = true;
    this.apiService
      .list('userrecipes', 0, 100000, value.id)
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (response: any) => this.mapRecipes(response.items),
        error: (error: any) =>
          this.messageService.showSnackBar(error.message, 'error'),
      });
  }
    ...
    if (recipe.moderated == '0') {
      this.apiService
        ...
        .subscribe({
          ...
          error: (error: any) =>
            this.messageService.showSnackBar('Hiba történt: ' + error, 'error'),
        });
    } else {
      this.apiService
          ...
          next: (response: any) => {
            if (response === null) {
              ...
              this.messageService.showSnackBar(
                'Recept sikeresen visszaállítva',
                'success'
              );
            }
          },
