...
export class FeaturedComponent implements OnInit {
  ...
  getLatest() {
    this.apiService.list('latest', 0, 4).subscribe({
      next: (response: any) => {
  ...
  getFree() {
    this.apiService.list('free', 0, 4).subscribe({
      next: (response: any) => {
  ...
  getDaily() {
    this.apiService.list('daily', 0, 4).subscribe({
      next: (response: any) => {
  ...
  getPopular() {
    this.apiService.list('popular', 0, 4).subscribe({
      next: (response: any) => {
