...
abstract class RecipeAPI
{
  ...
  public static function Put()
  {
    ...
    try {
      ...
      foreach ($recipe->labels as $key => $value) {
        $st->execute(array($recipe->getId(), $value['id']
        ));
      }
    ...
    $conn = null;
    $response = new Response(200, false, ["result" => "The recipe " . $recipe->recipeName . " has been updated.",
      "id" => $recipe->getId()]);
    return $response;
  }
  ...
  public static function Post()
  {
      ...
      $conn = null;
      $response = new Response(200, false, ["result" => "The recipe " . $recipe->recipeName . " has been uploaded.",
        "id" => $id]);
        return $response;
  }
  ...
  private static function removeFiles($original, $uploaded){
    foreach ($original as $key => $value) {
      if( array_search($value, $uploaded) === false &&
      $value !== '' && $value !== null){
        if(!unlink('../'.$value)){
          return false;
        }
